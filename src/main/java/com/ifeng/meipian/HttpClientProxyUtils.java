 ...
 */
public final class HttpClientProxyUtils {
    ...
    private static RequestConfig testRequestConfig = RequestConfig.custom().setConnectionRequestTimeout(WAIT_TIMEOUT)
            .setConnectTimeout(TEST_CONNECT_TIMEOUT).setSocketTimeout(TEST_READ_TIMEOUT).setProxy(new HttpHost("120.92.88.202",20004)).build();
    ...
    public static String sendGetRequestWithProxy(String url, Map<String, String> headParams, String decodeCharset,String ip,int port) throws Exception {
        long start = System.currentTimeMillis();
        String responseContent = null;
        RequestConfig testRequestConfig = RequestConfig.custom().setConnectionRequestTimeout(WAIT_TIMEOUT)
                .setConnectTimeout(TEST_CONNECT_TIMEOUT).setSocketTimeout(TEST_READ_TIMEOUT).setProxy(new HttpHost(ip,port)).build();
        HttpGet httpGet = new HttpGet(url);
        httpGet.setConfig(testRequestConfig);
        if (MapUtils.isNotEmpty(headParams)) {
            for (Map.Entry<String, String> entry : headParams.entrySet()) {
...
//            System.out.println(s);
            String s = HttpClientProxyUtils.sendGetParam("http://baobab.kaiyanapp.com/api/v4/categories/videoList?id=2", null, null);
            System.out.println(s);
